...
public:
    Koopa(const sf::Vector2f& position, float tileSize, KoopaKind kind = KoopaKind::Green, float initialSpeed = 60.f);
    virtual ~Koopa() override = default;

    void update(float deltaTime) override;
    virtual void update(float deltaTime, physics::PhysicsSystem& physicsSystem, const std::vector<physics::AABB>& solids, float mapWidth, float mapHeight);
    ...
    void stomp(bool playerFacingRight);
    ...
    [[nodiscard]] bool isShell() const noexcept { return m_state != KoopaState::Walking; }
...
protected:
    ...
    float m_shellSpeed{360.f};
    float m_animElapsed{0.f};
    int m_animationFrame{0};
    bool m_facingRight{false};

    void adjustColliderForState();
};
